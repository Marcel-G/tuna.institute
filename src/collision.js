...
export class Collision {
  ...
  static resolveCollision(collision) {
    ...
    const vap1 = Vector.add(
      a.v,
      Vector.crossA(a.av, r1)
    )
    const r2 = Vector.sub(collision.p, b.p);
    const vap2 = Vector.add(
      b.v,
      Vector.crossA(b.av, r2)
    )
    ...
    const totalMass = (1 / a.m) + (1 / b.m) + ((raxn * raxn) / a.i) + ((rbxn * rbxn) / b.i);
    const j = (-(1 + restitution) * Vector.dot(relativeVelocity, collision.n)) / totalMass;
  ...
  static correctPosition(collision) {
    ...
    a.p = Vector.sub(
      a.p,
      Vector.scale(collision.n, f * a.m * 0.8)
    );

    b.p = Vector.add(
      b.p,
      Vector.scale(collision.n, f * b.m * 0.8)
    );
  }
   ...
   */
  isEqual(entity) {
    return (this.a === entity.a && this.b === entity.b) ||
  ...
  renderDebug(delta, context) {
    ...
    context.lineTo(
      this.p.x - (this.n.x * this.d),
      this.p.y - (this.n.y * this.d)
    );
    context.stroke()
  }
}